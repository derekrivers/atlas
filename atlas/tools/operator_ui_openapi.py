...
def generate_runtime_enums(
    document: dict[str, Any],
    *,
    output_path: Path = GENERATED_RUNTIME_ENUMS,
) -> None:
    """Generate runtime enum values from the same OpenAPI schema."""

    schemas = cast(dict[str, Any], document["components"]["schemas"])
    enums = {
        name: cast(list[str], schema["enum"])
    ...
    content = "\n".join(
        [
            ...
            "",
            f"export const atlasOpenApiEnums = {json.dumps(enums, indent=2)} as const",
            "",
...
def check_committed_outputs() -> int:
    """Fail when regeneration changes committed operator UI generated files."""

    generated_paths = [
        GENERATED_CLIENT.relative_to(REPO_ROOT),
        GENERATED_RUNTIME_ENUMS.relative_to(REPO_ROOT),
    ]
    ...
    if result.returncode != 0 or status.stdout:
        relative_paths = ", ".join(str(path) for path in generated_paths)
        sys.stderr.write(
            "OpenAPI TypeScript generated output drift detected. "
            f"The regenerated files have been written to {relative_paths}; "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if output_arg is None:
            generate_runtime_enums(document)
    ...
    if args.check:
        return check_committed_outputs()
    return 0
