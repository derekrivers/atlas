...
- ATLAS-260's system-tier attestation assessment is FAIL. Its harness tested a
  conditional evaluator but synthesized the candidate mapping and simulated the
  external verifier; it did not exercise a trusted signer/OIDC lifecycle or
  independently prove that required jobs ran against the candidate. No
  candidate evidence path or session identity is authorised for this console.
...
assessments. ATLAS-260 failed to deliver candidate-pinned evidence and retires
the current no-rewrite approach. `head_sha` must itself contain the resolved
live `main` commit. A clean synthetic merge, candidate tree equality, an
assessment attestation fixture or a future squash tree relationship does not
substitute for that ancestry requirement. Mechanically stale sessions continue
to recover through the Phase 12 operator-owned rebase lane and restart as a new
